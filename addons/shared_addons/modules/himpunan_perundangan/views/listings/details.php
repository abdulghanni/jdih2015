							...
							<li><b><?=$entry->namasub.' Nomor '.$entry->title.' Tahun '.$entry->regyear?></b><br/>
								...
								<li class="item-description">
								  ...
								  <div class="view-detail">
									<a href="<?=site_url('himpunan_perundangan/download')?>/<?=$entry->entry_id?>">{{ theme:image file="green/Download.png" style="border:none;" }}</a>
									<a href="<?=site_url('himpunan_perundangan/listings/details')?>/<?=$entry->entry_id?>">{{ theme:image file="green/arrow_next.png" style="border:none;" }}</a>
								  </div>
								</li>
						<?php endforeach ?>
						</ul>
						&laquo; <?=anchor('himpunan_perundangan/category/'.$entry->FK_category_id, 'Kembali ke Kategori')?>
					</div>
    			</div>
			</div>
    </div>
