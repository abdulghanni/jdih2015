...
class Himpunan_perundangan extends Public_Controller {
    ...
    function index()
    {
		...
		$pagination = create_pagination('himpunan_perundangan/index/page/', $this->listings->count_all_entries(), $this->limit, 4);
    ...
    {							
		...
		{
			// Set pagination
			$pagination = create_pagination('himpunan_perundangan/category/'.intval($category), count($data['entries']), $this->limit, 4);
    ...
    function year($year = FALSE, $offset = FALSE)
    {
		...
		{						
			$pagination = create_pagination('himpunan_perundangan/year/'.intval($year), count($data['entries']), $this->limit, 4);
    ...
    function search($offset = FALSE)
    {
		...
		{
			// Set pagination
			$pagination = create_pagination('himpunan_perundangan/search/', $total_rows, $this->limit, 4);
	...
	function download($entry_id)
	{
		...
		$this->db->query("update default_ph_entries set downloaded=downloaded+1 where entry_id=".intval($entry_id) );
