...
class Listings extends MY_Controller
{
	...
	function offset($offset=FALSE)
	{
		if($_SERVER['REQUEST_METHOD']=='POST') {
			...
			$category = $this->session->userdata('adm_filter_kategori');
		}
	...
	function add($active = FALSE)
	{
		...
		$config['upload_path'] = '/var/www/html/v70/direktorihukum/public/download/';
		$config['allowed_types'] = 'pdf|doc|xls|docx|xlsx|ppt|pptx';
		...
		else
		{
			if ( !$this->upload->do_upload("url") )
			{
				...
				$this->session->set_flashdata('msg', $data['upload_data']);
				$this->template->display('member/listings/add', $data);
			}else{
