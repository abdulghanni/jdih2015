...
class Listings_model extends MY_Model
{
	...
	var $entries_table 		= 'default_ph_entries'; 	// Entries table
	var $categories_table 	= 'default_ph_categories'; 	// Category table
	...
	function get_all_Subcategories()
	{
		...
		$query = $this->db->get('ph_subcategories');
		return $query->result();
	}
	...
	{
		$this->db->select('f.name as namasub,c.description as c_description, e.description as e_description, ');		
		...
		$this->db->from($this->categories_table.' AS c');
		$this->db->join($this->entries_table.' AS e', 'c.category_id = e.FK_category_id', 'left');
		$this->db->join('ph_subcategories as f', 'f.category_id=e.sub_category_id','left');
	...
	{
		$this->db->select('c.description as c_description, e.description as e_description,e.sub_category_id,f.name as namasub, ');		
	...
	function count_search_result($nomor = FALSE, $tahun = FALSE, $tentang = FALSE, $params = array())
	{
		...
		if ($nomor!='') $sqlwhere .= " AND ".$this->db->dbprefix($this->entries_table).".title = '" . mysql_real_escape_string($nomor) . "'";
		if ($tahun!='') $sqlwhere .= " AND ".$this->db->dbprefix($this->entries_table).".regyear = '" . mysql_real_escape_string($tahun) . "'";
		if ($tentang!='') $sqlwhere .= " AND ".$this->db->dbprefix($this->entries_table).".description LIKE '%".mysql_real_escape_string($tentang)."%'";
	...
	function get_search_result($nomor = FALSE, $tahun = FALSE, $tentang = FALSE, $params = array())
	{
		...
		$this->db->select('f.name as namasub,entry_id, FK_category_id, FK_user_id, active, CAST('.$this->db->dbprefix($this->entries_table).'.title as SIGNED) as title, regyear, date_added, url');
		$this->db->select('hits, downloaded, ph_categories.category_id, ph_categories.name, ph_categories.entry_count');

		$this->db->from($this->categories_table);
		$this->db->join($this->entries_table, $this->db->dbprefix($this->categories_table).".category_id = ".$this->db->dbprefix($this->entries_table).".FK_category_id", "left");
		
		$this->db->join('ph_subcategories as f', 'f.category_id=ph_entries.sub_category_id','left');
		//print_r($sqlwhere);
		$sqlwhere = $this->db->dbprefix($this->entries_table).".entry_id` <> 0 ";
	...
	{	
		$this->db->select('c.description as c_description, e.description as e_description,f.name as namasub, ');		
		$this->db->select('e.entry_id, e.FK_category_id, e.FK_user_id, e.active, CAST(e.title as SIGNED) as title, e.regyear, e.date_added, e.url');
		$this->db->select('e.hits, e.downloaded, c.category_id, c.name, c.entry_count');
		$this->db->from($this->categories_table.' AS c');
		$this->db->join($this->entries_table.' AS e', 'c.category_id = e.FK_category_id', 'left');
	...
	function get_new_entries($limit = '5', $category = FALSE)
	{
		...
		$this->db->select('ph_subcategories.name as namasub,'.$this->categories_table.'.description AS cat_description, '.$this->entries_table.'.title as title,'.$this->entries_table.'.description, entry_id, CAST(title as SIGNED) as title, regyear, hits, downloaded, url');
		$this->db->join($this->categories_table, $this->entries_table.'.FK_category_id = '.$this->categories_table.'.category_id', 'left');		
		$this->db->join('ph_subcategories', 'ph_subcategories.category_id='.$this->entries_table.'.sub_category_id','left');
		$this->db->order_by('regyear desc, title desc');
		$this->db->limit($limit);
		$query = $this->db->get($this->entries_table);
		return $query->result();
