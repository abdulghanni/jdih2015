<div id="upload-box">
	<h2><?php echo lang('files.upload.title'); ?><span class="close ui-icon ui-icon-closethick">Close</span></h2>
		<?php echo form_open_multipart('admin/photo_activity/upload'); ?>
        ...
        <h3><?php echo lang('files.folders.label'); ?></h3>
            <ul id="files-nav">
                ...
                <li id="folder-id-<?php echo $folder->id; ?>" class="<?php echo $key == 0 ? 'current' : ''; ?>">
                    <?php echo anchor("index.php/photo_activity/admin_image/index/{$folder->id}", $folder->name, 'title="'.$folder->slug.'"'); ?>
                </li>
        ...
        <table class="table-list" border="0">
            ...
            <tbody>
                ...
                <tr class="<?php echo alternator('', 'alt'); ?>">
                    <td class="image"><img class="pyro-image" src="<?php echo base_url(); ?>uploads/files/<?php echo $file->filename; ?>" alt="<?php echo $file->name; ?>" width="50" onclick="javascript:insertImage('<?php echo $file->filename; ?>', '<?php echo htmlentities($file->id); ?>');" style="cursor:pointer" /></td>
