		...
		foreach ($links as $link)
		{
			...
			else
			{
				...
				if ( $indent )
				{
					...
					if ( $wrapper['children'] )
					{
						$output .= $ident_c . "<{$list_tag} class='dropdown-menu'>" . PHP_EOL;
				...
				else
				{
					...
					{
						if ($level == 0) {
							$att = ' class="dropdown-toggle" data-toggle="dropdown" role="button" ';
							$output .= (($level == 0) and $top == 'text' and $wrapper['children']) ? $item['title'] : anchor('javascript:void(0)', $item['title'].' <span class="caret"></span>', trim(implode(' ', $item['attributes'])).$att);
							$output .= "<{$list_tag} class='dropdown-menu'>";
						}else{
							$att = ' class="trigger right-caret" data-toggle="dropdown" role="button" ';
							$output .= (($level == 0) and $top == 'text' and $wrapper['children']) ? $item['title'] : '<a '.$att.' '.trim(implode(' ', $item['attributes'])).'>'.$item['title'].'</a>';
							$output .= "<{$list_tag} class='dropdown-menu sub-menu'>";
						}
